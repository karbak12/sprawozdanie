...
lapply(c("tidyverse", "e1071", "scales", "sf", "broom", "rnaturalearth", "ggsignif", "virdis"), library, character.only = TRUE)
```
...
1. Porównanie współczynnika Giniego dla państw bogatszych i biedniejszych

```{r, echo=FALSE, massage=FALSE, error=FALSE, warning=FALSE}
# Definiowanie list bogatych i biednych krajów
rich_countries <- data %>%
  filter(Kraj %in% c("Luxembourg", "Ireland", "Switzerland", "Norway"))

poor_countries <- data %>%
  filter(Kraj %in% c("Moldova", "Ukraine", "Belarus", "Bulgaria"))

# Wykres dla bogatych krajów
ggplot(rich_countries, aes(x = Rok, y = G, color = Kraj)) +
  geom_point() +
  labs(title = "Zmiany wskaźnika Giniego dla bogatszych państw",
       x = "Rok", y = "Wskaźnik Giniego") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  

# Wykres dla biednych krajów
ggplot(poor_countries, aes(x = Rok, y = G, color = Kraj)) +
  geom_point() +
  labs(title = "Zmiany wskaźnika Giniego dla biedniejszych państw",
       x = "Rok", y = "Wskaźnik Giniego") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  
...
2. Porównanie całej Europy co dziesięciolecie


```{r, echo=FALSE, massage=FALSE, error=FALSE, warning=FALSE}
# Ustawienie układu 2x2 dla wykresów
par(mfrow = c(2, 2))

# Wykres dla roku 1990
world_filtered_1990 <- world %>%
  filter(name %in% countries_to_show) %>%
  left_join(data_filtered %>% filter(Rok == 1990), by = c("name" = "Kraj"))

ggplot(world_filtered_1990) + 
  geom_sf(aes(fill = G), color = 'black') +  # Kolorowanie krajów na podstawie wskaźnika Giniego
  coord_sf(crs = st_crs(3035),               # Ustawienie układu współrzędnych
           xlim = c(2000000, 6200000), 
           ylim = c(1500000, 5500000)) +     # Zakres widocznej części mapy
  scale_fill_viridis_c(option = "C", na.value = 'white') +  # Skala kolorów i brak danych
  labs(title = "Mapa nierówności dochodowych w 1990 roku") +
  theme_minimal() +  # Minimalistyczny wygląd
  theme(axis.text = element_blank(),   # Usunięcie napisów osi
        axis.ticks = element_blank(),  # Usunięcie ticków osi
        axis.title = element_blank())  # Usunięcie tytułów osi
...
world_filtered_2000 <- world %>%
  filter(name %in% countries_to_show) %>%
  left_join(data_filtered %>% filter(Rok == 2000), by = c("name" = "Kraj"))

ggplot(world_filtered_2000) + 
  geom_sf(aes(fill = G), color = 'black') +  # Kolorowanie krajów na podstawie wskaźnika Giniego
  coord_sf(crs = st_crs(3035),               # Ustawienie układu współrzędnych
           xlim = c(2000000, 6200000), 
           ylim = c(1500000, 5500000)) +     # Zakres widocznej części mapy
  scale_fill_viridis_c(option = "C", na.value = 'white') +  # Skala kolorów i brak danych
  labs(title = "Mapa nierówności dochodowych w 2000 roku") +
  theme_minimal() +  # Minimalistyczny wygląd
  theme(axis.text = element_blank(),   # Usunięcie napisów osi
        axis.ticks = element_blank(),  # Usunięcie ticków osi
        axis.title = element_blank())  # Usunięcie tytułów osi

# Wykres dla roku 2010
world_filtered_2010 <- world %>%
  filter(name %in% countries_to_show) %>%
  left_join(data_filtered %>% filter(Rok == 2010), by = c("name" = "Kraj"))

ggplot(world_filtered_2010) + 
  geom_sf(aes(fill = G), color = 'black') +  # Kolorowanie krajów na podstawie wskaźnika Giniego
  coord_sf(crs = st_crs(3035),               # Ustawienie układu współrzędnych
           xlim = c(2000000, 6200000), 
           ylim = c(1500000, 5500000)) +     # Zakres widocznej części mapy
  scale_fill_viridis_c(option = "C", na.value = 'white') +  # Skala kolorów i brak danych
  labs(title = "Mapa nierówności dochodowych w 2010 roku") +
  theme_minimal() +  # Minimalistyczny wygląd
  theme(axis.text = element_blank(),   # Usunięcie napisów osi
        axis.ticks = element_blank(),  # Usunięcie ticków osi
        axis.title = element_blank())  # Usunięcie tytułów osi
...
world_filtered_2020 <- world %>%
  filter(name %in% countries_to_show) %>%
  left_join(data_filtered %>% filter(Rok == 2020), by = c("name" = "Kraj"))

ggplot(world_filtered_2020) + 
  geom_sf(aes(fill = G), color = 'black') +  # Kolorowanie krajów na podstawie wskaźnika Giniego
  coord_sf(crs = st_crs(3035),               # Ustawienie układu współrzędnych
           xlim = c(2000000, 6200000), 
           ylim = c(1500000, 5500000)) +     # Zakres widocznej części mapy
  scale_fill_viridis_c(option = "C", na.value = 'white') +  # Skala kolorów i brak danych
  labs(title = "Mapa nierówności dochodowych w 2020 roku") +
  theme_minimal() +  # Minimalistyczny wygląd
  theme(axis.text = element_blank(),   # Usunięcie napisów osi
        axis.ticks = element_blank(),  # Usunięcie ticków osi
        axis.title = element_blank())  # Usunięcie tytułów osi
