...
- Rok - rok w którym zostały zeprane dane.
Średnia: 2007
Mediana: 2009
...
top_countries <- data_filtered %>%
  ...
  arrange(desc(max_G)) %>%
  slice_head(n = 3) %>%
  pull(Kraj) # Pobierz nazwy krajów

# Wybór trzech krajów z najniższymi wartościami Giniego
bottom_countries <- data_filtered %>%
  group_by(Kraj) %>%
...
data_bottom <- data_filtered %>%
  filter(Kraj %in% bottom_countries)

# Wykres dla krajów z najwyższymi wartościami Giniego
plot_top <- ggplot(data_top, aes(x = Rok, y = G, color = Kraj)) +
  geom_line(size = 1) +
  labs(
    title = "Kraje z najwyższymi wartościami współczynnika Giniego",
    x = "Rok",
...
plot_bottom <- ggplot(data_bottom, aes(x = Rok, y = G, color = Kraj)) +
  geom_line(size = 1) +
  labs(
    title = "Kraje z najniższymi wartościami współczynnika Giniego",
    x = "Rok",
    y = "Współczynnik Giniego",
    color = "Kraj"
  ) +
...
```
Na podstawie wykresów można zauważyć, że kraje z najwyższymi wartościami współczynnika Giniego (Albania, Czarnogóra, Serbia) charakteryzują się znacznymi nierównościami dochodowymi, z widoczną zmiennością wartości w ostatnich latach. Z kolei kraje z najniższymi wartościami Giniego (Białoruś, Słowacja, Słowenia) wykazują stabilniejsze i niższe poziomy nierówności, co może świadczyć o efektywnej polityce redystrybucji dochodów.
